...
class DirectAssessmentDocLevelDAWindowTask(BaseMetadata):
    """
    ...
    def completed_items_for_user(self, user):
        results = DirectAssessmentDocLevelDAWindowResult.objects.filter(
    ...
    def next_item_for_user(self, user, return_completed_items=False):
        ...
        for item in self.items.all().order_by('id'):
            result = DirectAssessmentDocLevelDAWindowResult.objects.filter(
        ...
        if not next_item:
            ...
            annotations = DirectAssessmentDocLevelDAWindowResult.objects.filter(
    ...
    def next_document_for_user(self, user, return_statistics=True):
        ...
        completed_blocks = DirectAssessmentDocLevelDAWindowResult.objects.filter(
            task=self,
    ...
    def get_results_for_each_item(self, block_items, user):
        ...
        for item in block_items:
            result = (
                DirectAssessmentDocLevelDAWindowResult.objects.filter(
                    item__id=item.id,
                    completed=True,
    ...
    def import_from_json(cls, campaign, batch_user, batch_data, max_count):
        """
        Creates new Directassessmentdocleveldawindowtask instances based on JSON input.
        """
        ...
        for batch_task in batch_json:
            ...
            new_task = DirectAssessmentDocLevelDAWindowTask(
                campaign=campaign,
...
class DirectAssessmentDocLevelDAWindowResult(BaseMetadata):
    """
