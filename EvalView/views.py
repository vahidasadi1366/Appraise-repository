...
from EvalData.models import DirectAssessmentLongDocumentResult
from EvalData.models import DirectAssessmentLongDocumentTask
from EvalData.models import DirectAssessmentDocLevelDAWindowResult
from EvalData.models import DirectAssessmentDocLevelDAWindowTask
...
def direct_assessment_document(request, code=None, campaign_name=None):
    ...
    return render(request, "EvalView/direct-assessment-long-document.html", context)
...
# pylint: disable=C0103,C0330
@login_required
def direct_assessment_long_document(request, code=None, campaign_name=None):
    """
    Direct assessment long document annotation view.
    """
    t1 = datetime.now()
    ...
    # If language code has been given, find a free task and assign to user.
    if not current_task:
        current_task = DirectAssessmentLongDocumentTask.get_task_for_user(
            user=request.user
        )
    ...
    if not current_task:
        ...
        next_task = DirectAssessmentLongDocumentTask.get_next_free_task_for_language(
            code, campaign, request.user
        )
    ...
    if request.method == "POST":
        ...
        print( '*'* 150,
            score, item_id, task_id, document_id, start_timestamp, end_timestamp, ajax
        )
        LOGGER.info("score=%s, item_id=%s", score, item_id)
        print(
            "Got request score={0}, item_id={1}, ajax={2}".format(score, item_id, ajax)
        ...
        if score and item_id and start_timestamp and end_timestamp:
            ...
            if current_item.documentID == document_id:
                ...
                if current_item.itemID == int(item_id) and current_item.id == int(
                    ...
                    utc_now = datetime.utcnow().replace(tzinfo=utc)
                    # pylint: disable=E1101
                    DirectAssessmentLongDocumentResult.objects.create(
                        score=score,
                        start_time=float(start_timestamp),
                        end_time=float(end_timestamp),
                        item=current_item,
    ...
    # requests
    context = {
        "active_page": "direct-assessment-long-document",
        "item_id": current_item.itemID,
        "task_id": current_item.id,
        "document_id": current_item.documentID,
        "completed_blocks": completed_blocks,
...
@login_required
def single_direct_assessment_document(request, index=1, code=None, campaign_name=None):
    """
    Direct assessment document annotation view.
    """
    t1 = datetime.now()
    ...
    total_items = len(list(zip(block_items, block_scores)))

    if 1 < index < total_items + 1:
        next_page = f"http://127.0.0.1:8000/direct-assessment-document/{index + 1}"
        previous_page = f"http://127.0.0.1:8000/direct-assessment-document/{index - 1}"
    ...
    items_list = list(zip(block_items, block_scores))
    if 1 <= index <= len(items_list):
        page_context["items"] = items_list[index : index + 3]
    else:
        page_context[
            "items"
        ] = None  # Set items to None or an appropriate value when index is out of range
...
# pylint: disable=C0103,C0330
@login_required
def direct_assessment_DocLevelDA_Window(
    request, index=1, code=None, campaign_name=None
):
    """
    Direct assessment DocLevelDAWindow annotation view.
    """
    t1 = datetime.now()
    ...
    # If language code has been given, find a free task and assign to user.
    if not current_task:
        current_task = DirectAssessmentDocLevelDAWindowTask.get_task_for_user(
            user=request.user
        )
    ...
    if not current_task:
        ...
        next_task = (
            DirectAssessmentDocLevelDAWindowTask.get_next_free_task_for_language(
                code, campaign, request.user
            )
        )
        ...
        if score and item_id and start_timestamp and end_timestamp:
            ...
            if current_item.documentID == document_id:
                ...
                if current_item.itemID == int(item_id) and current_item.id == int(
                    ...
                    utc_now = datetime.utcnow().replace(tzinfo=utc)
                    # pylint: disable=E1101
                    DirectAssessmentDocLevelDAWindowResult.objects.create(
                        score=score,
                        start_time=float(start_timestamp),
                        end_time=float(end_timestamp),
                        item=current_item,
    ...
    # requests
    context = {
        "active_page": "direct-assessment-document-window",
        "item_id": current_item.itemID,
        "task_id": current_item.id,
        "document_id": current_item.documentID,
        "completed_blocks": completed_blocks,
    ...
    if 1 < index < total_items + 1:
        next_page = (
            f"http://127.0.0.1:8000/direct-assessment-document-window/{index + 1}/"
        )
        previous_page = (
            f"http://127.0.0.1:8000/direct-assessment-document-window/{index - 1}/"
        )
    ...
    return render(request, "EvalView/direct-assessment-document-window.html", context)
